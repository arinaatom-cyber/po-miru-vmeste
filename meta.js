window.ROUTE_META = {
  "karelia-yacht": {
    transport: ["train", "yacht"],
    transportLabel: "Поезд + яхта",
    category: "russia",
    image:
      "images/karelia-yacht.jpg",
    album: ["images/albums/karelia-yacht/photo-2.jpg","images/albums/karelia-yacht/photo-3.webp"],
    mapCenter: [59.5, 32.5],
    mapZoom: 5,
    stopCoords: {
      Сортавала: [61.7033, 30.6917],
      "Ладожские шхеры": [61.55, 30.55],
      "Санкт-Петербург": [59.9343, 30.3351],
      Шлиссельбург: [59.9436, 31.0333],
    },
  },
  "turkey-istanbul": {
    transport: ["plane", "car"],
    transportLabel: "Самолёт + авто",
    category: "turkey",
    image:
      "images/turkey-istanbul.jpg",
    album: ["images/albums/turkey-istanbul/photo-3.jpg","images/albums/turkey-istanbul/photo-4.png"],
    mapCenter: [38.5, 30.5],
    mapZoom: 6,
    stopCoords: {
      Анталия: [36.8969, 30.7133],
      Демре: [36.244, 29.987],
      Каш: [36.2, 29.637],
      Фетхие: [36.621, 29.116],
      Бодрум: [37.034, 27.43],
      Эфес: [37.94, 27.341],
      Бурса: [40.188, 29.061],
      Стамбул: [41.0082, 28.9784],
    },
  },
  "turkey-cappadocia": {
    transport: ["plane", "car"],
    transportLabel: "Самолёт + авто",
    category: "turkey",
    image:
      "images/turkey-cappadocia.jpg",
    album: ["images/albums/turkey-cappadocia/photo-1.jpg","images/albums/turkey-cappadocia/photo-3.jpg","images/albums/turkey-cappadocia/photo-4.jpg"],
    mapCenter: [37.6, 32.8],
    mapZoom: 6,
    stopCoords: {
      Анталия: [36.8969, 30.7133],
      Эгирдир: [37.8742, 30.8497],
      Конья: [37.8746, 32.4932],
      Каппадокия: [38.6431, 34.8289],
      Эрджиес: [38.5322, 35.4489],
      Кызкалеси: [36.4636, 34.1478],
      Мамуре: [36.0906, 32.9328],
      Каньоны: [37.15, 31.18],
      Кемер: [36.6021, 30.5595],
    },
  },
  "new-new-year": {
    transport: ["car"],
    transportLabel: "Авто",
    category: "russia",
    image:
      "images/new-new-year.jpg",
    album: ["images/albums/new-new-year/photo-1.jpg","images/albums/new-new-year/photo-2.png","images/albums/new-new-year/photo-3.jpg"],
    mapCenter: [54.5, 49.5],
    mapZoom: 5,
    stopCoords: {
      Москва: [55.7558, 37.6173],
      Владимир: [56.129, 40.4066],
      Казань: [55.7887, 49.1221],
      Самара: [53.1959, 50.1002],
      Тольятти: [53.5303, 49.3461],
      Муром: [55.575, 42.0515],
    },
  },
  "hainan-china": {
    transport: ["plane", "car"],
    transportLabel: "Самолёт",
    category: "china",
    image:
      "images/hainan-china.jpg",
    album: ["images/albums/hainan-china/photo-1.jpg","images/albums/hainan-china/photo-2.jpg"],
    mapCenter: [18.25, 109.5],
    mapZoom: 8,
    stopCoords: {
      Санья: [18.2528, 109.5119],
      Наньшань: [18.288, 109.2],
      Янода: [18.355, 109.562],
      "Дун Тянь": [18.293, 109.205],
      Пляжи: [18.22, 109.45],
    },
  },
  singapore: {
    transport: ["plane"],
    transportLabel: "Самолёт",
    category: "singapore",
    image:
      "images/singapore.jpg",
    mapCenter: [1.36, 103.82],
    mapZoom: 11,
    stopCoords: {
      Сингапур: [1.2838, 103.8591],
      Сентоза: [1.2494, 103.8303],
      "Ночное сафари": [1.4025, 103.7889],
      "Джохор-Бару": [1.4927, 103.7414],
    },
  },
  "sergiev-posad": {
    transport: ["car"],
    transportLabel: "Авто",
    category: "russia",
    image:
      "images/sergiev-posad.jpg",
    album: ["images/albums/sergiev-posad/photo-2.jpg","images/albums/sergiev-posad/photo-3.jpg"],
    mapCenter: [56.3, 38.1],
    mapZoom: 9,
    stopCoords: {
      Торбеево: [56.321, 38.272],
      "Сергиев Посад": [56.315, 38.136],
      Абрамцево: [56.237, 37.966],
      "Гремячий ключ": [56.281, 38.36],
      "Музеи города": [56.315, 38.136],
    },
  },
  vladivostok: {
    transport: ["plane", "car"],
    transportLabel: "Самолёт + внедорожник",
    category: "russia",
    image:
      "images/vladivostok.jpg",
    album: ["images/albums/vladivostok/photo-1.jpg","images/albums/vladivostok/photo-2.jpg","images/albums/vladivostok/photo-4.jpg"],
    mapCenter: [43.5, 132.5],
    mapZoom: 7,
    stopCoords: {
      Владивосток: [43.1155, 131.8855],
      "о. Русский": [43.006, 131.895],
      "Уссурийский залив": [43.15, 132.05],
      Дунай: [42.878, 132.337],
      Находка: [42.824, 132.892],
      "Бухта Ежовая": [42.95, 133.2],
      "Южное Приморье": [42.901, 133.904],
      Витязь: [42.598, 131.187],
    },
  },
  karelia: {
    transport: ["car"],
    transportLabel: "Авто",
    category: "russia",
    image:
      "images/karelia.jpg",
    album: [
      "images/albums/karelia/photo-2.png",
      "images/albums/karelia/photo-3.jpg",
      "images/albums/karelia/photo-4.png",
    ],
    mapCenter: [60.9, 32.8],
    mapZoom: 6,
    stopCoords: {
      "Санкт-Петербург": [59.9343, 30.3351],
      Петербург: [59.9343, 30.3351],
      Ладога: [61.02, 29.84],
      Онега: [61.78, 35.9],
    },
  },
  "spb-karelia": {
    transport: ["car"],
    transportLabel: "На машине",
    category: "russia",
    image:
      "images/spb-karelia.jpg",
    mapCenter: [61.0, 32.0],
    mapZoom: 6,
    stopCoords: {
      Петербург: [59.9343, 30.3351],
      Ладога: [61.02, 29.84],
      Онега: [61.78, 35.9],
      Кижи: [62.066, 35.225],
      Кинерма: [61.45, 33.65],
    },
  },
  sakhalin: {
    transport: ["plane", "car"],
    transportLabel: "Самолёт + авто",
    category: "russia",
    image:
      "images/sakhalin.jpg",
    album: ["images/albums/sakhalin/photo-1.jpg","images/albums/sakhalin/photo-2.png","images/albums/sakhalin/photo-3.jpg"],
    mapCenter: [47.0, 142.7],
    mapZoom: 7,
    stopCoords: {
      "Южно-Сахалинск": [46.959, 142.738],
      "Горные лыжи": [46.847, 142.766],
      Невельск: [46.683, 141.867],
      Корсаков: [46.634, 142.777],
      "Бухта Тихая": [48.249, 142.536],
    },
  },
  "spb-city": {
    transport: ["plane", "train"],
    transportLabel: "Самолёт / поезд",
    category: "russia",
    image:
      "images/spb-city.jpg",
    album: ["images/albums/spb-city/photo-1.jpg","images/albums/spb-city/photo-2.jpg","images/albums/spb-city/photo-3.jpg"],
    mapCenter: [59.934, 30.32],
    mapZoom: 13,
    stopCoords: {
      Центр: [59.9343, 30.3351],
      Эрмитаж: [59.9398, 30.3146],
      Коломна: [59.923, 30.305],
      Музеи: [59.9386, 30.3322],
      "Кафе и рестораны": [59.9343, 30.3351],
    },
  },
  "pack-essentials": {
    transport: ["checklist"],
    transportLabel: "Чек-лист",
    category: "checklist",
    image:
      "images/hero.jpg",
    mapCenter: [55.75, 37.62],
    mapZoom: 4,
    stopCoords: {},
  },
  "velo-yakhroma": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    image:
      "images/velo-yakhroma.jpg",
    mapCenter: [56.51, 37.33],
    mapZoom: 9,
    stopCoords: {
      Яхрома: [56.288, 37.487],
      Дмитров: [56.3444, 37.5203],
      Темпы: [56.6242, 37.1478],
      Карманово: [56.65, 37.2],
      Дубна: [56.733, 37.167],
    },
  },
  "velo-botsad-city": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    image:
      "images/velo-botsad-city.jpg",
    mapCenter: [55.78, 37.62],
    mapZoom: 11,
    stopCoords: {
      "Ботанический сад": [55.836, 37.601],
      "Лосиный остров": [55.875, 37.78],
      Автозаводская: [55.7074, 37.6568],
      "Москва-Сити": [55.7497, 37.5372],
    },
  },
  "velo-botsad-kolomenskoe": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    image:
      "images/velo-botsad-kolomenskoe.jpg",
    mapCenter: [55.75, 37.65],
    mapZoom: 11,
    stopCoords: {
      "Ботанический сад": [55.836, 37.601],
      Сокольники: [55.793, 37.675],
      Таганка: [55.741, 37.654],
      Коломенское: [55.6694, 37.6708],
    },
  },
  "velo-sergiev": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    image:
      "images/velo-sergiev.jpg",
    mapCenter: [56.03, 37.9],
    mapZoom: 9,
    stopCoords: {
      Москва: [55.7558, 37.6173],
      Мытищи: [55.9116, 37.7308],
      Пушкино: [56.0106, 37.8471],
      Хотьково: [56.2531, 37.9878],
      "Сергиев Посад": [56.315, 38.136],
    },
  },
  "velo-zvenigorod": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    image:
      "images/velo-zvenigorod.jpg",
    mapCenter: [55.74, 37.25],
    mapZoom: 10,
    stopCoords: {
      Москва: [55.7558, 37.6173],
      Рублёво: [55.7752, 37.3633],
      Успенское: [55.7192, 37.1072],
      Звенигород: [55.729, 36.856],
    },
  },
  "velo-konakovo": {
    transport: ["bike"],
    transportLabel: "На велосипеде",
    category: "velo",
    planned: true,
    image:
      "images/velo-konakovo.jpg",
    mapCenter: [56.72, 36.96],
    mapZoom: 10,
    stopCoords: {
      Конаково: [56.703, 36.762],
      Дубна: [56.733, 37.167],
    },
  },
};

window.FILTER_LABELS = {
  all: "Все",
  russia: "Россия",
  turkey: "Турция",
  china: "Китай",
  singapore: "Сингапур",
  car: "Авто",
  yacht: "Яхта",
  train: "Поезд",
  plane: "Самолёт",
  bike: "Велосипед",
};

/** Страна → города → маршруты */
window.PLACE_TREE = {
  russia: {
    label: "Россия",
    cities: [
      {
        id: "spb",
        label: "Санкт-Петербург",
        routes: ["spb-city"],
      },
      {
        id: "skerries",
        label: "Ладожские шхеры",
        routes: ["karelia-yacht"],
      },
      {
        id: "karelia",
        label: "Карелия",
        routes: ["karelia"],
      },
      {
        id: "sergiev",
        label: "Сергиев Посад",
        routes: ["sergiev-posad"],
      },
      {
        id: "samara",
        label: "Самара",
        routes: ["new-new-year"],
      },
      {
        id: "primorye",
        label: "Владивосток",
        routes: ["vladivostok"],
      },
      {
        id: "sakhalin",
        label: "Сахалин",
        routes: ["sakhalin"],
      },
    ],
  },
  turkey: {
    label: "Турция",
    cities: [
      {
        id: "antalya-istanbul",
        label: "Анталия — Стамбул",
        routes: ["turkey-istanbul"],
      },
      {
        id: "cappadocia",
        label: "Каппадокия",
        routes: ["turkey-cappadocia"],
      },
    ],
  },
  china: {
    label: "Китай",
    cities: [
      {
        id: "sanya",
        label: "Санья",
        routes: ["hainan-china"],
      },
    ],
  },
  singapore: {
    label: "Сингапур",
    cities: [
      {
        id: "singapore",
        label: "Сингапур",
        routes: ["singapore"],
      },
    ],
  },
};

/** Паспорт маршрута для карточек и шапки страницы */
window.ROUTE_PASSPORT = {
  "karelia-yacht": {
    days: "≈9 дней",
    distance: "Ладожские шхеры",
    mood: ["nature", "active"],
    tags: ["яхта-экспедиция"],
    pin: [61.55, 30.55],
    pinLabel: "Яхта-экспедиция",
  },
  "turkey-istanbul": {
    days: "≈10 дней",
    distance: "вдоль моря",
    mood: ["sea", "city", "food"],
    tags: ["авто"],
    pin: [38.5, 30.5],
    pinLabel: "Турция · Анталия—Стамбул",
  },
  "turkey-cappadocia": {
    days: "≈13 дней",
    distance: "кольцо",
    mood: ["nature", "sea", "active"],
    tags: ["авто", "Каппадокия"],
    pin: [38.64, 34.83],
    pinLabel: "Турция · Каппадокия",
  },
  "new-new-year": {
    days: "несколько дней",
    distance: "Москва — Самара",
    mood: ["city", "weekend"],
    tags: ["авто"],
    pin: [55.79, 49.12],
    pinLabel: "Поездка в Самару",
  },
  "hainan-china": {
    days: "≈7–10 дней",
    distance: "Санья",
    mood: ["sea", "food", "nature"],
    tags: ["пляж", "дайвинг"],
    pin: [18.25, 109.5],
    pinLabel: "Санья",
  },
  singapore: {
    days: "≈5–7 дней",
    distance: "Сингапур — Джохор-Бару",
    mood: ["city", "food"],
    tags: ["самолёт"],
    pin: [1.35, 103.82],
    pinLabel: "Сингапур",
  },
  "sergiev-posad": {
    days: "2–3 дня",
    distance: "Торбеево озеро",
    mood: ["weekend", "nature", "city"],
    tags: ["база отдыха"],
    pin: [56.32, 38.14],
    pinLabel: "Сергиев Посад · Торбеево",
  },
  vladivostok: {
    days: "≈10 дней",
    distance: "Приморье",
    mood: ["sea", "active", "nature"],
    tags: ["острова", "маяки", "бухты"],
    pin: [43.12, 131.89],
    pinLabel: "Приморье · внедорожник",
  },
  karelia: {
    days: "≈7–12 дней",
    distance: "Ладога и Онега",
    mood: ["nature", "active", "food"],
    tags: ["авто"],
    pin: [61.0, 33.0],
    pinLabel: "СПб · Карелия",
  },
  sakhalin: {
    days: "≈8 дней",
    distance: "Южный Сахалин",
    mood: ["nature", "active", "sea"],
    tags: ["лыжи", "рыбалка", "бухты"],
    pin: [46.96, 142.74],
    pinLabel: "Южный Сахалин",
  },
  "spb-city": {
    days: "выходные / несколько дней",
    distance: "город",
    mood: ["city", "food"],
    tags: ["Эрмитаж", "кафе", "квартиры"],
    pin: [59.93, 30.33],
    pinLabel: "Санкт-Петербург",
  },
  "velo-yakhroma": {
    days: "с ночёвкой",
    distance: "канал им. Москвы",
    mood: ["active", "nature"],
    tags: ["вело", "Карманово"],
  },
  "velo-botsad-city": {
    days: "вечер",
    distance: "будни",
    mood: ["active", "city"],
    tags: ["вело"],
  },
  "velo-botsad-kolomenskoe": {
    days: "день",
    mood: ["active", "city"],
    tags: ["вело"],
  },
  "velo-sergiev": {
    days: "день",
    distance: "к Лавре",
    mood: ["active", "city"],
    tags: ["вело"],
  },
  "velo-zvenigorod": {
    days: "день",
    mood: ["active", "nature"],
    tags: ["вело"],
  },
  "velo-konakovo": {
    days: "в планах",
    mood: ["active", "nature"],
    tags: ["вело"],
  },
};

window.TOP_PICKS = [
  {
    id: "karelia-yacht",
    label: "Вернулись бы завтра",
    reason: "",
  },
  {
    id: "hainan-china",
    label: "Лучшее море",
    reason: "",
  },
  {
    id: "turkey-cappadocia",
    label: "Лучший road trip",
    reason: "",
  },
];

